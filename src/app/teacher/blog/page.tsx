  ...
  const fetchBlogPosts = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Filter only current teacher's posts
        const myPosts = data.blogPosts.filter((post: BlogPost) => post.author.id === session?.user?.id)
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-lg p-8">
          <div className="flex justify-between items-center mb-8">
            <h1 className="text-3xl font-bold text-black">ჩემი ბლოგები</h1>
            <button
              onClick={() => {
                setShowForm(true)
                setEditingPost(null)
